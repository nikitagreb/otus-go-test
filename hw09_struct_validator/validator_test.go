...
type (
	User struct {
		...
		BloodType int      `validate:"in:1,2,3,4"`
	...
	tests := []struct {
		...
		},
		{
			...
			in: User{
				...
				Email:     "email",
				Role:      "foo",
				Phones:    "1234567890111",
			},
			expectedErr: ValidationErrors{},
			expectedErrMsg: "invalid value: value length (7) is not match required length (36), " +
				"invalid value: value (55) is more than (50), invalid value: value (6) is not in (1,2,3,4), " +
				"invalid value: value (email) is not matched regexp (^\\w+@\\w+\\.\\w+$), " +
				"invalid value: value (foo) is not in (admin,stuff), " +
				"invalid value: value length (13) is not match required length (11)",
		},
		{
			...
			expectedErrMsg: "invalid value: value (1) is less than (18), invalid value: value (0) is not in (1,2,3,4), " +
				"invalid value: value length (0) is not match required length (11)",
		},
		{
			name: "invalid rules",
			in: Dog{
